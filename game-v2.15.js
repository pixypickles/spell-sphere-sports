...
function finishCup(){simulateCpu('rush','guard');simulateCpu('guard','shoot');simulateCpu('shoot','rush');$('finalStandings').innerHTML=tableHTML();const place=sortedTable().findIndex(r=>r.id==='player')+1;$('cupEndTitle').textContent=place===1?'ビギナーカップ優勝！':`ビギナーカップ ${place}位`;$('cupEndText').textContent=place===1?'次ランクへの挑戦権を獲得しました。':'もう一度挑戦できます。';$('cupEndPanel').classList.remove('hidden')}
...
function ai(u,dt,isEnemy){
  ...
  if(danger&&now-u.lastDodge>1.9&&Math.random()<.08){
    u.lastDodge=now;u.dodgeT=.32;u.inv=.23;n={x:-danger.dy,y:danger.dx};
  }
...
$('dodgeBtn').addEventListener('pointerdown',e=>{
  ...
  let x=input.x,y=input.y;if(Math.hypot(x,y)<.15){x=1;y=0}
...
bindTap('cupStartBtn',()=>{mode='cup';cupIndex=0;cupTable=newCupTable();currentOpponent=CUP_ORDER[0];$('menu').classList.add('hidden');refreshCup();$('cupPanel').classList.remove('hidden')});
...
bindTap('cupBackBtn',()=>{mode='menu';$('cupPanel').classList.add('hidden');$('menu').classList.remove('hidden')});
bindTap('cupFinishBtn',()=>{mode='menu';$('cupEndPanel').classList.add('hidden');$('menu').classList.remove('hidden')});
bindTap('nextBtn',()=>{$('result').classList.add('hidden');const ended=bScore>=2||rScore>=2;if(!ended){round++;reset();return}if(mode==='cup'){recordMatch('player',CUP_ORDER[cupIndex],bScore,rScore);cupIndex++;if(cupIndex>=3)finishCup();else{refreshCup();$('cupPanel').classList.remove('hidden')}}else{mode='menu';$('menu').classList.remove('hidden')}});
...
window.__gameDebug=()=>({mode,cupIndex,currentOpponent,enemies:enemies.length,aliveEnemies:enemies.filter(e=>e.alive).length,menuHidden:$('menu').classList.contains('hidden'),cupHidden:$('cupPanel').classList.contains('hidden'),running});
})();