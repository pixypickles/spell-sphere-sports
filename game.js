...
function updateOrientation(){
  ...
  if(landscape)forcePortraitPlay=false;
...
document.addEventListener('click',e=>{
  const btn=e.target&&e.target.closest?e.target.closest('#portraitContinueBtn'):null;
  if(btn){
    forcePortraitPlay=true;
    document.body.classList.add('forcePortraitPlay');
    updateOrientation();
  }
});
...
function drawUnit(u){
  // v3.11: genuine frog mages never use human headwear/outfit head pieces.
  ...
  if(u.fairyActive){
    ...
    // v3.11 fairy appearance: human/fairy face, blue hair, twin buns, point eyes, ▼ mouth
    g.fillStyle='#f2cf72';
