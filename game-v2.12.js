...
function reset(){
  ...
  const roles=TEAMS[selectedTeam].roles;
  enemies=[
  ...
  flash(TEAMS[selectedTeam].name,900);
}
...
function finish(team,text){
  ...
  setTimeout(()=>{
    ...
    $('nextBtn').textContent=match?'最初に戻る':'次のラウンド';
...
function syncTeam(){
  ...
  $('teamDesc').textContent=TEAMS[selectedTeam].desc;
}
$('teamSelect').addEventListener('change',syncTeam);syncTeam();
...
$('startBtn').addEventListener('pointerup',e=>{e.preventDefault();$('startBtn').click()});

$('nextBtn').addEventListener('click',()=>{
  $('result').classList.add('hidden');
  if(bScore>=2||rScore>=2){bScore=0;rScore=0;round=1;score.textContent='0 - 0';$('menu').classList.remove('hidden')}
  else{round++;reset()}
});
