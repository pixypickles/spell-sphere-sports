...
class Unit{
  constructor(x,y,team,controlled=false,role='balance'){
    Object.assign(this,{x,y,team,controlled,role,r:17,alive:true,mana:100,lastShot:-9,shotCd:.85,lastDodge:-9,dodgeT:0,inv:0,dodgeRecover:0,dx:0,dy:0,emote:0,think:0,target:null,charging:false,chargeT:0,curveSide:1,rollAngle:0});
...
function ai(u,dt,isEnemy){
  ...
  if(u.role==='attacker'){
    if(t&&dist(u,t)<155){tx=u.x-(t.x-u.x)*.65;ty=u.y-(t.y-u.y)*.35;}
    else{
      const passedMid=isEnemy?u.x<690:u.x>590;
      if(!passedMid){tx=isEnemy?805:475;ty=u.y<CY?225:465;}
      else{tx=enemyFlag.x+(isEnemy?-1:1)*-55;ty=enemyFlag.y;}
    }
  }
